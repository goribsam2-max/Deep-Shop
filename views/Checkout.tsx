...
const Checkout: React.FC<CheckoutProps> = ({ user }) => {
  ...
  const handleWhatsAppVerification = () => {
    ...
    const message = encodeURIComponent(`আমি আমার NID/জন্ম নিবন্ধন এবং আমার মা/বাবার NID ও নম্বর দিচ্ছি, আমার অর্ডারটি কনফার্ম করুন।`);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (paymentMode === 'advance') {
        ...
        orderData.trustProof = trustData;
      }

      const docRef = await addDoc(collection(db, 'orders'), orderData);
      
      const productList = cartItems.map((p: any) => `• ${p.quantity}x ${p.name}`).join('\n');
      let msg = `📦 <b>New Order Received! (${paymentMode === 'advance' ? 'ADVANCE' : 'TRUST MODE'})</b>\n`;
      ...
      if (paymentMode === 'advance') {
        ...
        msg += `<b>Trust Proof Provided:</b>\nID Type: ${trustData.idType}\nParent: ${trustData.parentName}\nParent Phone: ${trustData.parentPhone}\n`;
      }
      ...
      notify('Order registered successfully!', 'success');
    ...
    return (
      <div className="max-w-2xl mx-auto p-8 py-20 text-center animate-fade-in">
        ...
        <p className="text-slate-500 mb-10 leading-relaxed font-medium">
          আপনার অর্ডারটি সফলভাবে রিকোয়েস্ট করা হয়েছে। এখন ভেরিফিকেশনের জন্য নিচের বাটনে ক্লিক করে আমাদের WhatsApp-এ আপনার প্রয়োজনীয় ডকুমেন্টস গুলো জমা দিন।
        </p>
        <button 
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-12 pb-40 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          ...
          <section className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-white/5 p-8 rounded-2xl shadow-sm">
            <h2 className="font-bold text-xs uppercase tracking-widest mb-8 text-slate-400">02. Payment Verification Option</h2>
            ...
            <div className="flex gap-4 mb-10">
              ...
              >
                ...
                <span className="opacity-60 text-[7px]">৳300 Booking Fee</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="opacity-60 text-[7px]">Trust Proof Mode</span>
              </button>
            </div>

            {paymentMode === 'advance' ? (
              <div className="animate-fade-in">
                <p className="text-sm text-slate-500 mb-6 font-medium">To confirm your order, please send a booking advance of ৳300 to our personal number: <b className="text-primary">01778953114</b></p>
                ...
                <div>
                  ...
                  <input placeholder="ENTER TXN ID" className="w-full h-14 px-6 bg-slate-50 dark:bg-black/20 rounded-xl font-mono text-center font-black text-xl tracking-widest border border-primary/10 focus:border-primary transition-all outline-none" value={transactionId} onChange={e => setTransactionId(e.target.value)} />
                </div>
              </div>
            ) : (
              <div className="animate-fade-in space-y-6">
                <div className="p-5 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-2xl mb-6">
                  <p className="text-[11px] text-red-600 dark:text-red-400 font-bold leading-relaxed uppercase tracking-tight">
                    <i className="fas fa-info-circle mr-2"></i>
                    কনো অ্যাডভান্স দেওয়া ছাড়া অর্ডার করতে হলে আপনার NID অথবা জন্ম নিবন্ধন (Birth Certificate) এর সাথে আপনার বাবা বা মা যেকোনো একজনের NID এবং তার সাথে আপনার বাবা বা মার নম্বর দিন।
                  </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Identity Type</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Parent Name (Father/Mother)</label>
                    <input placeholder="Enter Name" className="w-full h-12 px-5 bg-slate-50 dark:bg-black/20 border border-slate-100 dark:border-white/10 rounded-xl outline-none font-bold text-xs" value={trustData.parentName} onChange={e => setTrustData({...trustData, parentName: e.target.value})} />
                  </div>
                  <div className="col-span-full">
                    <label className="text-[10px] font-bold uppercase text-slate-400 mb-2 block">Parent Mobile Number</label>
        ...
        <div className="bg-slate-900 text-white p-8 rounded-[32px] h-max lg:sticky lg:top-24 shadow-2xl">
           ...
           <div className="pt-6 border-t border-white/10 mb-8 flex justify-between items-center">
              <span className="text-xs font-bold uppercase text-slate-400">Final Total</span>
           ...
           >
             {loading ? <i className="fas fa-spinner animate-spin"></i> : 'Register Order'}
           </button>
